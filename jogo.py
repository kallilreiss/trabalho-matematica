...
def gerar_pergunta(dificuldade):
    ...
    # múltiplos de 5
    v = random.randrange(100, 2001, 5)
    t = random.randrange(5, 101, 5)
    ...
    if tipo == "desconto":
        res = round(v*(1-t/100))
        return f"R${v} com {t}% de desconto → valor final?", res, f"Resposta: {res}"

    # ACRÉSCIMO
    if tipo == "acrescimo":
        res = round(v*(1+t/100))
        return f"R${v} com aumento de {t}% → valor final?", res, f"Resposta: {res}"

    # JUROS SIMPLES
    if tipo == "juros_simples":
        res = round(v*(1 + (t/100)*tempo))
        return f"R${v} a {t}% por {tempo} meses (simples)", res, f"Resposta: {res}"

    # JUROS COMPOSTOS
    if tipo == "juros_compostos":
        res = round(v*((1+t/100)**tempo))
        return f"R${v} a {t}% por {tempo} meses (composto)", res, f"Resposta: {res}"

    # COMPARAÇÃO (não óbvia)
    if tipo == "comparacao":

        t1 = random.randrange(10, 31, 5)
        t2 = t1 + random.choice([5, 10])

        tempo1 = random.randint(1, 3)
...
Opção 1: {t1}% por {tempo1} meses  
Opção 2: {t2}% por {tempo2} meses  

Qual dá mais dinheiro? (1 ou 2)
"""
        return pergunta, correta, f"Resposta correta: {correta}"
# ---------- MENU ----------
...
# ---------- JOGO ----------
    ...
    st.markdown(f"### 💵 R${round(st.session_state.dinheiro)} | Rodada {st.session_state.rodada}/10")
    ...
    if st.button("Confirmar resposta"):
        ...
        if resp_val == st.session_state.r:
